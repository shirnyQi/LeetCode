...
func minSpeedOnTime(dist []int, hour float64) int {
	...
	leftSpeed, rightSpeed := 1, math.MaxInt32

	for leftSpeed < rightSpeed {
		mid := (leftSpeed + rightSpeed) / 2
		var cost float64
		for _, v := range dist[:n-1] {
			if v%mid == 0 {
				cost += float64(v / mid)
			} else {
				cost += float64(v/mid) + 1
			}
		}
...
func TestName(t *testing.T) {
	...
	//fmt.Println(minSpeedOnTime([]int{1, 1, 100000}, 2.01)) // 10000000
	fmt.Println(minSpeedOnTime([]int{1, 1}, 1.0)) // -1
}
